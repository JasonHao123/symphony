...
* {@link CustomRedisOperationsSessionRepository} will then translate the Redis message into a
...
* {@link org.springframework.session.data.redis.CustomRedisOperationsSessionRepository.RedisSession#getMaxInactiveIntervalInSeconds()}
* . For example:
* </p>
*
* <pre>
...
* @since 1.0
*/
public class CustomRedisOperationsSessionRepository implements
		FindByIndexNameSessionRepository<CustomRedisOperationsSessionRepository.RedisSession>,
		MessageListener {
	private static final Log logger = LogFactory
			.getLog(CustomRedisOperationsSessionRepository.class);
	...
	@Value("${spring.session.redis.module-session-attribute}")
	String MODULE_SESSION_ATTR_PREFIX;
	
	@Value("#{'${spring.session.redis.global-session-keys}'.split(',')}") 
	List<String> globalSessionKeys;
	...
	private final CustomRedisSessionExpirationPolicy expirationPolicy;
	 ...
	 */
	public CustomRedisOperationsSessionRepository(
	...
	public CustomRedisOperationsSessionRepository(
		...
		this.expirationPolicy = new CustomRedisSessionExpirationPolicy(sessionRedisOperations,
				this);
	}
		...
		for (Map.Entry<Object, Object> entry : entries.entrySet()) {
			...
			if (CREATION_TIME_ATTR.equals(key)) {
				...
				loaded.setAttribute(key.substring(SESSION_ATTR_PREFIX.length()),
						entry.getValue());
			}else if(key.startsWith(MODULE_SESSION_ATTR_PREFIX)) {
				loaded.setAttribute(key.substring(MODULE_SESSION_ATTR_PREFIX.length()),
						entry.getValue());
			}
		}
		return loaded;
	...
	String getSessionAttrNameKey(String attributeName) {
		String prefix = MODULE_SESSION_ATTR_PREFIX;
		if(globalSessionKeys.contains(attributeName)) {
			prefix = SESSION_ATTR_PREFIX;
		}
	 ...
	 * {@link org.springframework.session.data.redis.CustomRedisOperationsSessionRepository.RedisSession#saveDelta()}
	...
	final class RedisSession implements ExpiringSession {
		...
		private void flushImmediateIfNecessary() {
			if (CustomRedisOperationsSessionRepository.this.redisFlushMode == RedisFlushMode.IMMEDIATE) {
				saveDelta();
			}
		}
		...
		private void saveDelta() {
			...
			if (this.delta.containsKey(principalSessionKey)
				...
				if (this.originalPrincipalName != null) {
					...
					CustomRedisOperationsSessionRepository.this.sessionRedisOperations
			...
			CustomRedisOperationsSessionRepository.this.expirationPolicy
