...
public class ${name}Producer extends DefaultProducer {
    ...
    public void process(Exchange exchange) throws Exception {
		...
		String service = ((${name}Endpoint)getEndpoint()).getRemaining();
		${name}Service targetService = getEndpoint().getCamelContext().getRegistry().lookupByNameAndType("${scheme}Service",${name}Service.class);
		Method method = ${name}Service.class.getMethod(service, Exchange.class);
